    ...
    <div className="space-y-6">
      <div role="list" className="space-y-3">
        {data.map(edu => (
          <div
            key={edu.id}
            className={`p-4 border rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700 ${draggingId === edu.id ? 'opacity-60' : ''}`}
            draggable
            onDragStart={e => {
              e.dataTransfer.effectAllowed = 'move';
              e.dataTransfer.setData('text/plain', edu.id);
              setDraggingId(edu.id);
            }}
            onDragOver={e => {
              e.preventDefault();
            ...
            onDrop={e => {
              ...
              if (!sourceId || sourceId === edu.id) {
                setDraggingId(null);
                return;
              }
              const from = data.findIndex(d => d.id === sourceId);
              const to = data.findIndex(d => d.id === edu.id);
            ...
            role="listitem"
            tabIndex={0}
            aria-label={`${t.education} item`}
          >
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center">
                <GripVertical className="h-5 w-5 mr-2 text-gray-400" aria-hidden />
                <h3 className="font-medium dark:text-white">{t.education}</h3>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div>
                <label className="flex items-center">
                  ...
                  />
                  <span className="text-sm text-gray-700 dark:text-gray-200">{t.inProgress}</span>
                </label>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-200">
